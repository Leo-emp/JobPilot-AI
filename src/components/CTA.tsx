/* ============================================================
   CTA SECTION - Final Call to Action
   ============================================================
   A large, bold section near the bottom of the page that
   encourages visitors to sign up. Fades up on scroll with
   a staggered reveal — heading first, then text, then buttons.
   ============================================================ */

"use client";

import Link from "next/link";
import { motion } from "framer-motion";

/* # Smooth deceleration curve */
const EASE: [number, number, number, number] = [0.16, 1, 0.3, 1];

/* # Parent — staggers children for sequential reveal */
const stagger = {
  hidden: {},
  show: { transition: { staggerChildren: 0.12 } },
};

/* # Child — each element fades up smoothly */
const fadeUp = {
  hidden: { opacity: 0, y: 18 },
  show: {
    opacity: 1,
    y: 0,
    transition: { duration: 0.9, ease: EASE },
  },
};

export default function CTA() {
  return (
    <section className="relative z-10 py-24 sm:py-32 px-4">
      <motion.div
        variants={stagger}
        initial="hidden"
        whileInView="show"
        viewport={{ once: true, margin: "-80px" }}
        className="max-w-4xl mx-auto text-center"
      >

        {/* # Decorative gradient background glow behind the CTA */}
        <div
          className="absolute inset-0 mx-auto w-full sm:w-[800px] h-[300px] sm:h-[500px] -z-10"
          style={{
            background:
              "radial-gradient(ellipse, rgba(56,189,248,0.1) 0%, rgba(56,189,248,0.03) 40%, transparent 65%)",
          }}
        />

        {/* # Main CTA heading */}
        <motion.h2 variants={fadeUp} className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold mb-6">
          Ready to{" "}
          <span className="glow-text-strong">Launch Your Career?</span>
        </motion.h2>

        {/* # Supporting text */}
        <motion.p variants={fadeUp} className="text-lg sm:text-xl text-text-secondary mb-10 max-w-2xl mx-auto leading-relaxed">
          Join thousands of professionals who use JobPilot AI to land interviews
          faster, write better resumes, and walk into every opportunity fully prepared.
        </motion.p>

        {/* # CTA Buttons */}
        <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-4 justify-center">
          <Link href="/signup" className="btn-primary text-lg px-10 py-4">
            Get Started Free →
          </Link>
          <Link href="/#features" className="btn-secondary text-lg px-10 py-4">
            Learn More
          </Link>
        </motion.div>

        {/* # Trust line */}
        <motion.p variants={fadeUp} className="mt-8 text-base text-text-muted">
          Free plan available &bull; No credit card required &bull; Cancel
          anytime
        </motion.p>
      </motion.div>
    </section>
  );
}
