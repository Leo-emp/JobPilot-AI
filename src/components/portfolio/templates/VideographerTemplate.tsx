...
/* # Videographer Template — Cinematic widescreen, dark film aesthetic, play button overlays */
...
function Letterbox() {
  return (
    <>
      <div className="w-full h-6 md:h-10" style={{ backgroundColor: "#000000" }} />
    </>
  );
}
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
      <motion.div className="w-16 h-16 rounded-full flex items-center justify-center backdrop-blur-md"
        style={{ backgroundColor: "rgba(229,9,20,0.85)", boxShadow: `0 0 30px ${c.red}40` }}
...
/* # Film-style card */
function FilmCard({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <motion.div
      className={`rounded-xl overflow-hidden ${className}`}
      style={{ backgroundColor: c.card, border: `1px solid ${c.border}` }}
      whileHover={{ y: -4, boxShadow: `0 8px 40px rgba(229,9,20,0.08)`, transition: { duration: 0.2 } }}
    >
      {children}
    </motion.div>
  );
            ...
            {avatarUrl && (
              <motion.div className="shrink-0"
                ...
                <img src={avatarUrl} alt={data.userName}
                  ...
                  style={{ boxShadow: c.shadow, border: `2px solid ${c.border}` }} />
              </motion.div>
            )}
            <div className="flex-1">
              {/* # Director credit style */}
              <motion.p className="text-xs uppercase tracking-[0.3em] mb-4 font-medium"
                style={{ color: c.red }}
                initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }}>
                {data.tagline || "Director · Cinematographer · Editor"}
              </motion.p>

              <motion.h1 className="text-5xl md:text-7xl lg:text-8xl font-extrabold tracking-tighter leading-none"
                style={{ color: c.text }}
              ...
              {about && "bio" in about && about.bio && (
                <motion.p className="mt-6 text-base leading-relaxed max-w-xl" style={{ color: c.muted }}
  ...
  return (
    <div className="mb-12">
      <div className="flex items-center gap-4 mb-3">
        ...
        {subtitle && <span className="text-xs uppercase tracking-[0.2em]" style={{ color: c.red }}>{subtitle}</span>}
      </div>
      <h2 className="text-3xl font-extrabold tracking-tight" style={{ color: c.text }}>{title}</h2>
    </div>
  );
}
              ...
              <motion.div key={i} variants={staggerItem}>
                <FilmCard>
                  <div className="flex flex-col md:flex-row">
                    {/* # Widescreen thumbnail with play button */}
                    ...
                    <div className="flex-1 p-8 flex flex-col justify-center">
                      <span className="text-xs uppercase tracking-widest mb-2" style={{ color: c.red }}>Project {String(i + 1).padStart(2, "0")}</span>
                      <h3 className="text-2xl font-bold" style={{ color: c.text }}>{p.title}</h3>
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                          {p.techStack.map((t, j) => (
                            <span key={j} className="text-xs px-3 py-1 rounded-full"
                              style={{ backgroundColor: c.redSoft, color: c.red, border: `1px solid ${c.red}20` }}>
                              {t}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="mt-5 flex gap-4">
                        ...
                        {p.repoUrl && (
                          <a href={p.repoUrl} target="_blank" rel="noopener noreferrer"
                            className="text-sm font-semibold px-5 py-2 rounded-lg transition-all hover:opacity-80"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {section.entries.map((t, i) => (
              <FilmCard key={i} className="p-7">
                <div className="text-3xl mb-3" style={{ color: c.red, opacity: 0.4 }}>"</div>
