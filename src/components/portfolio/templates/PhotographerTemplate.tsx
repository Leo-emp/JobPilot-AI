...
function renderSection(section: PortfolioSection) {
  switch (section.type) {
                ...
                <div className="w-full md:w-3/5 rounded-xl overflow-hidden group" style={{ boxShadow: c.shadow, border: `1px solid ${c.border}` }}>
                  {hasVideo(p) ? (
                    ...
                    <div className="aspect-[4/3] flex items-center justify-center" style={{ backgroundColor: c.surface }}>
                      <svg width="40" height="40" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5} style={{ color: c.border }}><path strokeLinecap="round" strokeLinejoin="round" d="M6.827 6.175A2.31 2.31 0 015.186 7.23c-.38.054-.757.112-1.134.175C2.999 7.58 2.25 8.507 2.25 9.574V18a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18V9.574c0-1.067-.75-1.994-1.802-2.169a47.865 47.865 0 00-1.134-.175 2.31 2.31 0 01-1.64-1.055l-.822-1.316a2.192 2.192 0 00-1.736-1.039 48.774 48.774 0 00-5.232 0 2.192 2.192 0 00-1.736 1.039l-.821 1.316z" /><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 12.75a4.5 4.5 0 11-9 0 4.5 4.5 0 019 0zM18.75 10.5h.008v.008h-.008V10.5z" /></svg>
                    </div>
                  )}
                </div>
    ...
    /* # ─── EXPERIENCE — Compact side-by-side horizontal strips ─── */
    case "experience":
      if (section.entries.length === 0) return null;
      return (
            ...
            {section.entries.map((e, i) => {
              ...
              return (
                <motion.div key={i}
                  ...
                  style={{ background: `linear-gradient(135deg, ${c.surface}, ${c.card})`, border: `1px solid ${c.border}` }}
                  whileHover={{ y: -2, boxShadow: `0 8px 25px rgba(0,0,0,0.3)` }}
                  initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }} transition={{ delay: i * 0.08 }}>
                  <div className="absolute left-0 top-0 bottom-0 w-0.5"
                    style={{ background: `linear-gradient(180deg, ${gradient.from}50, ${gradient.to}15)` }} />

                  <div className="p-4 pl-4.5">
                    <div className="flex items-center justify-between gap-3 mb-2">
                      <div className="min-w-0">
                        <h3 className="text-sm font-bold truncate" style={{ color: c.text }}>{e.title}</h3>
                        <p className="text-xs" style={{ color: c.accent, opacity: 0.7 }}>
                          {e.company}{e.location ? ` · ${e.location}` : ""}
                        </p>
                      </div>
                      {(e.startDate || e.endDate) && (
                        <span className="text-[10px] font-medium uppercase tracking-wider shrink-0 px-2 py-0.5 rounded-full"
                          style={{ background: `${gradient.from}10`, color: gradient.from, border: `1px solid ${gradient.from}12` }}>
