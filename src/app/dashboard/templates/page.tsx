...
function buildCompact(d: ResumeData): string {
  const css = `
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    h2 { font-size:10px; font-weight:800; text-transform:uppercase; letter-spacing:1.5px; color:#fff; background:#374151; padding:7px 10px 5px; margin-bottom:5px; display:inline-block; text-align:center; line-height:1; }
...
function buildStandardATS(d: ResumeData): string {
  ...
  if (d.experience) {
    ...
    html += entries.map(e => {
      ...
      let row = `<div class="std-entry"><div class="entry-row"><span class="left">${esc(leftText)}</span>`;
