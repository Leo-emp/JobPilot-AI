/* ============================================================
   MESSAGES — Candidate's messaging inbox
   ============================================================
   Lists all message threads with employers, sorted by recency.
   Click a thread to view and reply to messages.
   ============================================================ */
...
export default function MessagesPage() {
  ...
  const [selectedEmployer, setSelectedEmployer] = useState<{ name: string; logoUrl: string | null } | null>(null);
  ...
  const openThread = useCallback(async (threadId: string) => {
    ...
    setSelectedEmployer(thread?.employer ?? null);

    try {
      const res = await fetch(`/api/user/messages/${threadId}`);
  ...
  async function handleBlock() {
    ...
    if (!confirm("Block this conversation? You won't receive messages from this employer.")) return;

    try {
      await fetch(`/api/user/messages/${selectedThread}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      {threads.length === 0 ? (
        <div className="rounded-xl border border-slate-700/50 bg-slate-800/50 p-12 text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">
            When you have mutual interest with an employer, messaging will be unlocked here.
          </p>
        </div>
      ) : (
        <div className="flex gap-4 rounded-xl border border-slate-700/50 bg-slate-800/50 overflow-hidden" style={{ height: "calc(100vh - 240px)" }}>
          {/* # Thread list (left panel) */}
          <div className="w-80 flex-shrink-0 border-r border-slate-700/50 overflow-y-auto">
            {threads.map((thread) => {
              const preview = thread.messages[0];
              const isUnread = preview && preview.senderType === "employer" && !preview.readAt;
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className={`font-medium truncate ${isUnread ? "text-white" : "text-slate-300"}`}>
                        {thread.employer.name}
                      </div>
                      {preview && (
                        <p className="mt-0.5 text-xs text-slate-500 truncate">
                          {preview.senderType === "candidate" ? "You: " : ""}
              ...
              <>
                ...
                <div className="flex items-center justify-between border-b border-slate-700/50 px-4 py-3">
                  <div className="font-medium text-white">
                    {selectedEmployer?.name ?? "Employer"}
                  </div>
                  <button
                    onClick={handleBlock}
                      ...
                      >
                        ...
                        >
                          ...
                          <p className={`mt-1 text-xs ${
                            msg.senderType === "candidate" ? "text-indigo-300" : "text-slate-500"
                          }`}>
